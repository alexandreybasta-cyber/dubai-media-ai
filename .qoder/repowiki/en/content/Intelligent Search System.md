...
## Update Summary
**Changes Made**
- Enhanced SearchIndex metadata fields with title, thumbnail, and persons arrays
- Updated search results display to show comprehensive metadata including thumbnails and person names
- Modified orchestrator to include person identification in searchable segments
- Updated frontend components to render richer search results with thumbnails and person information
...
Router->>Orchestrator : search(query, top_k)
Orchestrator->>SearchIndex : search(query, top_k)
SearchIndex-->>Orchestrator : results[] (enhanced metadata)
Orchestrator-->>Router : results[]
Router-->>Client : {query, results, total}
```
...
- **SearchDemo Component**: Shows thumbnails, person names, and formatted timestamps
...
- Missing thumbnails: Verify thumbnail paths are properly extracted from ingestion results and accessible to the frontend.
...
**Section sources**
- [backend/pipeline/search_index.py:173-182](file://backend/pipeline/search_index.py#L173-L182)
- [backend/pipeline/search_index.py:246-255](file://backend/pipeline/search_index.py#L246-L255)
- [frontend/src/lib/useVideoProcessing.ts:77-84](file://frontend/src/lib/useVideoProcessing.ts#L77-L84)